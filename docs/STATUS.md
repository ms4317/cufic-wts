...
- **거래 타이머(`RoundTimer.jsx`: `TimerPill`+`TimerEdge`)** — **A안** 헤더 필(라벨+시간+얇은 진행바, 학생·관리자 공통) + **C안** 헤더 하단 전폭 라인(학생, 교실 뒤에서도 체감). 여유=골드 / 60초↓=경고(`--warn` 주황) / 30초↓=긴급(빨강+점 깜빡임+테두리 발광) + 30초 토스트(1회). 두 테마 확인. 진행바 기준값=`round_duration_seconds`.
...
1. ~~**타이머 시작 길이가 UI에서 10분 하드코딩**~~ → **해결(2026-08-20).** [타이머 시작]이 `startTimer(durMin)`으로
   `round_duration_seconds`(게임 설정 값)를 실제로 쓴다. 진행바 기준값도 동일. 타이머 리디자인(A/C안)과 함께 배선 완료.
